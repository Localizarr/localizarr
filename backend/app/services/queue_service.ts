...
export class QueueService {
  ...
  private pollingLimit = 5
  private queueDir = 'tmp/queues'
  private readonly MAX_PAYLOAD_BYTES = 256 * 1024
  ...
  public async publish(topic: string, data: any): Promise<void> {
    ...
    if (Buffer.byteLength(payload, 'utf8') > this.MAX_PAYLOAD_BYTES) {
      console.warn(`[QueueService] Payload exceeds ${this.MAX_PAYLOAD_BYTES} bytes, truncating responseData`)
      // ponytail: truncate responseData to keep queue usable instead of dropping message
      if (data.responseData) {
        const str = JSON.stringify(data.responseData)
        data.responseData = JSON.parse(str.substring(0, this.MAX_PAYLOAD_BYTES))
      }
    }
    console.log(`[QueueService] Publishing to topic: ${topic}`)
    try {
  ...
  public async start(): Promise<void> {
    ...
    console.log('[QueueService] Starting queue processing with 1s interval')
    this.intervalId = setInterval(() => {
      this.processQueues()
    }, 1000)

    console.log('[QueueService] Started processing queues every 1s')
  }
