...
export class ProxyService {
  ...
  private handleConnection(socket: net.Socket, apiKey?: string) {
    ...
    let requestLine = ''
    const MAX_BUFFER_BYTES = 64 * 1024 // ponytail: cap to avoid Slow-loris OOM

    this.logger.debug(`Iniciando processamento de nova conexão`)

    socket.on('data', (data) => {
      if (buffer.length + data.length > MAX_BUFFER_BYTES) {
        this.logger.warn('Buffer cap exceeded, closing socket')
    ...
    socket.setTimeout(40000) // 40s: Prowlarr 30s + margin for LLM (ponytail: tune if Prowlarr SLA changes)
  }
  ...
  private handleConnect(socket: net.Socket, requestLine: string) {
    ...
    targetSocket.setTimeout(35000)
    targetSocket.on('timeout', () => {
      this.logger.warn(`Timeout ao conectar ${host}:${port}`)
  ...
  private relayTraffic(socket1: net.Socket, socket2: net.Socket) {
    ...
    const cleanup = () => {
      socket1.unpipe(socket2)
      socket2.unpipe(socket1)
    }
