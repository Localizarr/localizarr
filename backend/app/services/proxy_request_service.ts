...
export class ProxyRequestService {
  ...
  // Prowlarr can take up to 30s — timeout must be above that to avoid false 502
  private readonly PROWLARR_TIMEOUT_MS = 35000
  private readonly MAX_CACHE_BODY_BYTES = 512 * 1024 // 512KB cap to avoid GB SQLite
  ...
  async proxyRequest(
    ...
    const response = await axios({
      ...
      timeout: this.PROWLARR_TIMEOUT_MS,
   ...
   * ponytail: 512KB cap per entry; larger bodies skipped to avoid GB SQLite
   */
  private async cacheResponseAsync(
    ...
    try {
      ...
      if (Buffer.byteLength(bodyStr, 'utf8') > this.MAX_CACHE_BODY_BYTES) {
        console.log(`[ProxyRequestService] Skipping cache: body ${bodyStr.length} chars exceeds cap`)
        return
      }
